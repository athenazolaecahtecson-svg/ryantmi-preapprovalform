// ============================================================
//  Toyota Makati Inc. Pre-Approval System — JavaScript
// ============================================================

const ADMIN_EMAIL = 'villagomez144@gmail.com';
const ADMIN_PASS  = 'Tmi@112233';

let submissions = JSON.parse(localStorage.getItem('tmi_submissions') || '[]');
...
function buildDoc(data) {
  ...
  return `
    <div class="print-header">
      <div class="print-logo-area">
        <div class="print-brand">
          <h1>Toyota Makati Inc.</h1>
        </div>
      </div>
      <div class="print-title">
        <h2>PRE-APPROVAL APPLICATION</h2>
        <p>${nowStamp(ts)}</p>
        <p>Ref: TMI-${ts.replace(/[^0-9]/g, '').slice(2, 12)}</p>
      </div>
    </div>
    ...
    <div class="print-footer">
      <p>Toyota Makati Inc. | This document is confidential and for authorized use only.</p>
      <div class="timestamp-area">
        <span>Generated by TMI Online Pre-Approval System</span>
...
function submitForm() {
  ...
  localStorage.setItem('tmi_submissions', JSON.stringify(submissions));
      ...
      <div class="admin-card">
        <div class="admin-card-info">
          ...
          <p style="font-size:0.78rem;color:#aaa;">
            Ref: TMI-${s.ts.replace(/[^0-9]/g, '').slice(2, 12)}
          </p>
        </div>
        <div class="admin-card-actions">
          <button class="btn-view" onclick="viewSubmission(${i})">View / Print</button>
          <button class="btn-delete" onclick="deleteSubmission(${i})">🗑 Delete</button>
        </div>
      </div>`;
  }).join('');
}
...
function deleteSubmission(i) {
  ...
  const name = [s.borrower.first, s.borrower.last].filter(Boolean).join(' ') || 'this client';
